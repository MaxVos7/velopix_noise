...
def plot_scan(filename, dacMin, dacMax):
    ...
    target = np.mean(
        0.5 * (data_Trim0[(data_Trim0 > 0) & (data_TrimF > 0)] + data_TrimF[(data_Trim0 > 0) & (data_TrimF > 0)]))
    ...
    plt.text(dacMax + 20, math.exp(8), mytext, fontsize=12, horizontalalignment='left', verticalalignment='center',
             color='red')
    mytext = "F Trim:\n%.1f +/- %.1f" % (np.mean(data_TrimF[data_TrimF > 0]), np.std(data_TrimF[data_TrimF > 0]))
    plt.text(dacMax + 20, math.exp(7), mytext, fontsize=12, horizontalalignment='left', verticalalignment='center',
             color='blue')
    mytext = "Target:\n%.1f" % target
    plt.text(dacMax + 20, math.exp(6), mytext, fontsize=12, horizontalalignment='left', verticalalignment='center',
             color='black')
    mytext = "Predicted:\n%.1f +/- %.1f" % (np.mean(data_Equal[data_Equal > 0]), np.std(data_Equal[data_Equal > 0]))
    plt.text(dacMax + 20, math.exp(5), mytext, fontsize=12, horizontalalignment='left', verticalalignment='center',
             color='black')
    if (addCont):
        mytext = "Achieved:\n%.1f +/- %.1f" % (
        np.mean(data_Control[data_Control > 0]), np.std(data_Control[data_Control > 0]))
        plt.text(dacMax + 20, math.exp(4), mytext, fontsize=12, horizontalalignment='left', verticalalignment='center',
                 color='green')

    mytext = "Masked:\n%d" % (nMasked)
    plt.text(dacMax + 20, math.exp(2), mytext, fontsize=12, horizontalalignment='left', verticalalignment='center',
             color='black')
...
def plot_mask(filename):
    ...
    plt.legend(bbox_to_anchor=(1.042, 0.3), loc=2, borderaxespad=0, handles=[catA, catB, catC, catD, catE, catT],
...
def plot_trim(filename):
    ...
    plt.legend(bbox_to_anchor=(1.042, 0.075), loc=2, borderaxespad=0, handles=[red_patch], frameon=False,
               handlelength=1.25)
...
def plot_noise_histogram(filename, trim):
    ...
    xmin = 4
    xmax = 10
    edges = [i / 10.0 for i in
             range(10 * xmin, 10 * xmax + 1)]  # set the x-axis edges of the histogram from xmin to xmax in steps of 0.1
