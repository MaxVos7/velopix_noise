...
def make_mask_map(predictions_to_map: list, mask_type: str = None):
    ...
    if mask_type != None:
        ...
        mpatches.Patch(color=cmap(0),
                       label=f"Inside 2 std: {np.count_nonzero(outside_2std_total == 0)}")
    ...
    plt.legend(bbox_to_anchor=(1.042, 0.8), loc=2, borderaxespad=0, handles=patches,
...
make_mask_map(list(map(lambda value: format(value, 'x').upper(), np.arange(1, 15))), 'Outside_5Std_Not_Dead')
